...
func (user *User) Save() rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to get last insert id", errors.New("database error"))
	}
...
func (user *User) Update() rest_errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to update save user statement", err)
...
func (user *User) Delete() rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to prepare delete user", errors.New("database error"))
	}
	defer stmt.Close()
...
func (user *User) FindByStatus(status string) ([]User, rest_errors.RestErr) {
	...
	if err != nil {
		logger.Error("error when trying to find users by id", err)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); err != nil {
			...
			return nil, rest_errors.NewInternalServerError("error when scan user into user struct", errors.New("database error"))
		}
...
func (user *User) FindByEmailAndPassword() rest_errors.RestErr {
	...
	if err != nil {
		...
		return rest_errors.NewInternalServerError("error when trying to prepare get user by email and password", errors.New("database error"))
	}
	defer stmt.Close()
